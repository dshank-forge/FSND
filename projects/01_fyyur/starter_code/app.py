...
def show_venue(venue_id):
  ...
  class udacity_venue(object):
      def __init__(self, venue_record):
        ...
        past_shows_query = db.session.query(Show).join(Venue).filter(Show.venue==venue_id).filter(Show.datetime<=current_datetime).all()
        self.past_shows = [udacity_show(sh) for sh in past_shows_query]
        upcoming_shows_query = db.session.query(Show).join(Venue).filter(Show.venue==venue_id).filter(Show.datetime>current_datetime).all()
        self.upcoming_shows = [udacity_show(sh) for sh in upcoming_shows_query]
